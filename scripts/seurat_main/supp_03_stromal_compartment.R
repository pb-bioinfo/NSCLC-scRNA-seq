...
s3a <- DimPlot(obj_Fibro, reduction = "umap.endo.rpca", label=T, cols = col_vector,group.by = "seurat_clusters",label.size = 4,repel = FALSE) + NoLegend()+
  labs(y = "UMAP_2", x = "UMAP_1")+ggtitle("")



ggsave(filename = "s3a.png", plot = s3a, width = 3.4, height = 3, units = "in")
...
cluster.averages <- AggregateExpression(obj_Fibro,group.by = "seurat_clusters",return.seurat = TRUE)
...
ggsave(filename = "s3b.png", plot = s3b, width = 6.5, height = 9, units = "in")
...
cluster.averages <- AggregateExpression(obj_Endo,group.by = "seurat_clusters",return.seurat = TRUE)
